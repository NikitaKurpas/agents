...
update-skills:
    ./scripts/update_skills.ts --file skills/.vendor/sources.toml --dest skills/.vendor --overwrite
...
sync-skills:
    ...
    # Snapshot all repo skill dirs, excluding container directories.
    find "{{ justfile_directory() }}/skills" -mindepth 1 -maxdepth 1 -type d ! -name '.*' -exec basename {} \; > "$repo_skills_tmp"
    find "{{ justfile_directory() }}/skills/.vendor" -mindepth 1 -maxdepth 1 -type d -exec basename {} \; >> "$repo_skills_tmp"
    sort -u -o "$repo_skills_tmp" "$repo_skills_tmp"
    ...
    while IFS= read -r skill || [ -n "$skill" ]; do
      ...
      [ -d "$src" ] || src="{{ justfile_directory() }}/skills/.vendor/$skill"
